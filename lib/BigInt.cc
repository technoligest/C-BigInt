...
bool BigInt::operator>(const BigInt &int2) const{
  ...
  throw Exception(Exception::ErrorTypes::INTERNAL_ERROR, __LINE__, __FILE__,
...
bool BigInt::operator==(const BigInt &int2) const{
  if(fullInteger_.size() != int2.fullInteger_.size()){
    return false;
  }
  if(isPositive_ != int2.isPositive_){
    return false;
  }
  for(int i = 0; i < fullInteger_.size(); ++i){
    if(fullInteger_[i] != int2.fullInteger_[i]){
      return false;
    }
  }
  return true;
...
string BigInt::divByTwo(const string &s) const{
  ...
  for(const char &ch:s){
    if(!std::isdigit(ch)){
      throw Exception(Exception::USER_ERROR, __LINE__, __FILE__,
                      "The input integer cannot have a non-integer character.");
    }
...
ull BigInt::stringToUll(const string s) const{
  ...
  for(int i = 0; i < s.size(); ++i){
    if(s[i] != '1' && s[i] != '0'){
      throw Exception(Exception::USER_ERROR, __LINE__, __FILE__,
                      "Cannot convert string to unsigned long long when it contains a non-binary charcater. Could be internal or usage error.");
    }
    result <<= 1;
    if(s[i] == '1'){
      result |= 1;
